...
(defpackage wernicke 
   ...
   (:export "DEFPARSER" "PARSE" "CHR" "STR" "ONE-OF" "NONE-OF" 
      "ANY" "MANY" "SEQ" "CHOICE" "RANGE" "TRY" "OPTIONAL" "SEP" "SEP-ANY" "TIMES"
      "*RUN-HOOKS*"))
...
(define-parser choice (&rest parsers)
   (iter (for parser in parsers)
         (for parser-result = (run-parser (try parser)))
         (on-success parser-result (leave (run-parser parser)))
...
(define-parser sep (parser by)
	(iter (for parse-result = (run-parser parser))
			(when (or (error? parse-result) (error? sep-result)) (finish))
			(collect parse-result into result)
			(for sep-result = (run-parser by))
			(when (or (error? parse-result) (error? sep-result)) (finish))
			(finally (return (or result (fail 'sep))))))

(define-parser sep-any (parser by)
	(let ((v (run-parser (sep parser by))))
			(if (error? v) nil v)))
